  ...
  if (Scheduler(&AltitudeHoldControlTimer, SCHEDULER_SET_FREQUENCY(50, "Hz")))
  {
    ...
    if ((Do_Altitude_Hold || Do_RTH_Or_Land_Call_Alt_Hold) && IS_STATE_ACTIVE(PRIMARY_ARM_DISARM))
    {
      ...
      if (Do_RTH_Or_Land_Call_Alt_Hold)
      {
        ...
        if (GPSParameters.Mode.Navigation == DO_LAND_IN_PROGRESS || GPSParameters.Mode.Navigation == DO_LAND_DETECTED || GPSParameters.Mode.Navigation == DO_LANDED)
        {
          if (HoveringState)
          {
            HoveringState = false;
          ...
          {
            TargetVariometer += (int32_t)(250 - MinVariometer) * (Barometer.INS.Altitude.Estimated - ConvertCMToMeters(SafeAltitude)) / (ConvertCMToMeters(GPSParameters.Home.Altitude) - ConvertCMToMeters(SafeAltitude));
          }
...
void ApplyAltitudeHoldPIDControl(uint16_t DeltaTime, bool HoveringState)
{
  ...
  VariometerErrorISum += ((VariometerError * GET_SET[PID_ALTITUDE].kI * DeltaTime) >> 7) / ((HoveringState && ABS(TargetVariometer) < 100) ? 2 : 1);
  ...
  int16_t VarioPIDControl = ((VariometerError * GET_SET[PID_ALTITUDE].kP) >> 5) + VariometerErrorIPart -
                            (((int32_t)INS.AccelerationEarthFrame_Filtered[2] * GET_SET[PID_ALTITUDE].kD) >> 6);
